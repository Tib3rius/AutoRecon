...
def cprint(
    ...
    **kvargs
    ...
    for attempt in range(10):
        try:
            fmted = string.Formatter().vformat(
                unfmt, args, vals
            )
            break
...
with open(
    os.path.join(
        rootdir, "config", port_scan_profiles_config_file
    ),
    "r",
) as p:
    try:
...
with open(
    os.path.join(rootdir, "config", "service-scans.toml"),
    "r",
) as c:
    try:
...
with open(
    os.path.join(rootdir, "config", "global-patterns.toml"),
    "r",
) as p:
    try:
...
if "password_wordlist" in service_scans_config:
    if isinstance(
        ...
        password_wordlist = service_scans_config[
            "password_wordlist"
        ]


async def read_stream(
    stream, target, tag="?", patterns=[], color=Fore.BLUE
):
            ...
            for p in global_patterns:
                ...
                if "description" in p:
                    for match in matches:
                        if verbose >= 1:
                            info(
                                "Task {bgreen}{tag}{rst} on {byellow}{address}{rst} - {bmagenta}"
                                + p["description"].replace(
                                    "{match}",
                                    "{bblue}{match}{crst}{bmagenta}",
                                )
                                + "{rst}"
                            )
                    ...
                    for match in matches:
                        if verbose >= 1:
                            info(
                                "Task {bgreen}{tag}{rst} on {byellow}{address}{rst} - {bmagenta}Matched Pattern: {bblue}{match}{rst}"
                            )
                        async with target.lock:
                            with open(
                                os.path.join(
...
async def run_cmd(
    semaphore, cmd, target, tag="?", patterns=[]
):
    async with semaphore:
        ...
        info(
            "Running task {bgreen}{tag}{rst} on {byellow}{address}{rst}"
            + (
                " with {bblue}{cmd}{rst}"
                if verbose >= 1
                else ""
        ...
        async with target.lock:
            with open(
                os.path.join(scandir, "_commands.log"), "a"
            ) as file:
    ...
    if process.returncode != 0:
        error(
            "Task {bred}{tag}{rst} on {byellow}{address}{rst} returned non-zero exit code: {process.returncode}"
        )
        async with target.lock:
            with open(
                os.path.join(scandir, "_errors.log"), "a"
            ) as file:
                file.writelines(
                    e(
                        "[*] Task {tag} returned non-zero exit code: {process.returncode}. Command: {cmd}\n"
                    )
                )
    else:
        info(
            "Task {bgreen}{tag}{rst} on {byellow}{address}{rst} finished successfully in {elapsed_time}"
        )
...
async def parse_service_detection(
    stream, tag, target, pattern
):
    address = target.address
    services = []
...
async def run_portscan(
    ...
    async with semaphore:
        ...
        if port_scan is not None:
            ...
            info(
                "Running port scan {bgreen}{tag}{rst} on {byellow}{address}{rst}"
                + (
                    " with {bblue}{command}{rst}"
                    if verbose >= 1
                    else ""
                )
            )
            ...
            async with target.lock:
                with open(
                    os.path.join(scandir, "_commands.log"),
                    "a",
                ) as file:
            ...
            output = [
                parse_port_scan(
                    process.stdout, tag, target, pattern
                ),
                read_stream(
                    process.stderr,
                    target,
            ...
            if process.returncode != 0:
                ...
                info(
                    "Port scan {bgreen}{tag}{rst} on {byellow}{address}{rst} finished successfully in {elapsed_time}"
                )
        ...
        info(
            "Running service detection {bgreen}{tag}{rst} on {byellow}{address}{rst}"
            + (
                " with {bblue}{command}{rst}"
                if verbose >= 1
                else ""
        ...
        if process.returncode != 0:
            ...
            async with target.lock:
                with open(
                    os.path.join(scandir, "_errors.log"),
                    "a",
                ) as file:
                    file.writelines(
                        e(
...
async def start_heartbeat(target, period=60):
    while True:
        ...
        async with target.lock:
            ...
            if count > 1:
                info(
                    "{bgreen}[{current_time}]{rst} - There are {byellow}{count}{rst} tasks still running on {byellow}{target.address}{rst}"
                    + tasks_list
                )
            elif count == 1:
                info(
                    "{bgreen}[{current_time}]{rst} - There is {byellow}1{rst} task still running on {byellow}{target.address}{rst}"
                    + tasks_list
                )
...
async def scan_services(loop, semaphore, target):
    ...
    heartbeat = loop.create_task(
        start_heartbeat(target, period=heartbeat_interval)
    )
    ...
    for profile in port_scan_profiles_config:
        if profile == port_scan_profile:
            for scan in port_scan_profiles_config[profile]:
                service_detection = (
                    port_scan_profiles_config[profile][
                        scan
                ...
                if (
                    "port-scan"
                    in port_scan_profiles_config[profile][
                        scan
                    ]
                ):
                    port_scan = (
            ...
            if result["returncode"] == 0:
                if result["name"] == "run_portscan":
                    for service_tuple in result["services"]:
                        ...
                        info(
                            "Found {bmagenta}{service}{rst} on {bmagenta}{protocol}/{port}{rst} on target {byellow}{address}{rst}"
                        )
                        ...
                        if not only_scans_dir:
                            with open(
                                ...
                                file.writelines(
                                    e(
                                        "[*] {service} found on {protocol}/{port}.\n\n\n\n"
                                    )
                                )
                        ...
                        if service.startswith(
                            "ssl/"
                        ) or service.startswith("tls/"):
                            service = service[4:]

                        for (
                            service_scan
                        ) in service_scans_config:
                                            ...
                                            if (
                                                ...
                                                if (
                                                    ...
                                                    warn(
                                                        ...
                                                        + tag
                                                        + Style.NORMAL
                                                        + "] Scan cannot be run against {protocol} port {port}. Skipping."
                                                        + Fore.RESET
                                                    )
                                                    continue
                                            ...
                                            if (
                                                ...
                                                if (
                                                    ...
                                                    warn(
                                                        ...
                                                        + address
                                                        + Style.NORMAL
                                                        + "] Scan should only be run once and it appears to have already been queued. Skipping."
                                                        + Fore.RESET
                                                    )
                                                    continue
                                                else:
                                                ...
                                                if (
                                                    ...
                                                    warn(
                                                        ...
                                                        + address
                                                        + Style.NORMAL
                                                        + "] Scan appears to have already been queued, but it is not marked as run_once in service-scans.toml. Possible duplicate tag? Skipping."
                                                        + Fore.RESET
                                                    )
                                                    continue
                                                else:
...
def scan_host(target, concurrent_scans):
    ...
    if single_target:
        ...
        basedir = os.path.abspath(
            os.path.join(outdir, target.address + srvname)
        )
    target.basedir = basedir
    os.makedirs(basedir, exist_ok=True)

    if not only_scans_dir:
        exploitdir = os.path.abspath(
            os.path.join(basedir, "exploit")
        )
        os.makedirs(exploitdir, exist_ok=True)

        lootdir = os.path.abspath(
            os.path.join(basedir, "loot")
        )
        os.makedirs(lootdir, exist_ok=True)

        reportdir = os.path.abspath(
            os.path.join(basedir, "report")
        )
        ...
        open(
            os.path.abspath(
                os.path.join(reportdir, "local.txt")
            ),
            "a",
        ).close()
        open(
            os.path.abspath(
                os.path.join(reportdir, "proof.txt")
            ),
            "a",
        ).close()

        screenshotdir = os.path.abspath(
            os.path.join(reportdir, "screenshots")
        )
        os.makedirs(screenshotdir, exist_ok=True)

    scandir = os.path.abspath(
    ...
    os.makedirs(
        os.path.abspath(os.path.join(scandir, "xml")),
        exist_ok=True,
    )
    ...
    try:
        loop.run_until_complete(
            scan_services(loop, semaphore, target)
        )
        elapsed_time = calculate_elapsed_time(start_time)
        info(
            "Finished scanning target {byellow}{target.address}{rst} in {elapsed_time}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        default="default",
        help="The port scanning profile to use (defined in port-scan-profiles.toml). Default: %(default)s",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Only scan a single target. A directory named after the target will not be created. Instead, the directory structure will be created within the output directory. Default: false",
    )
    parser.add_argument(
        "--only-scans-dir",
        action="store_true",
        default=False,
        help='Only create the "scans" directory for results. Other directories (e.g. exploit, loot, report) will not be created. Default: false',
    )
    parser.add_argument(
        "--heartbeat",
        action="store",
        type=int,
        default=60,
        help="Specifies the heartbeat interval (in seconds) for task status messages. Default: %(default)s",
    )
    nmap_group = parser.add_mutually_exclusive_group()
    nmap_group.add_argument(
        "--nmap",
        action="store",
        default="-vv --reason -Pn",
    ...
    nmap_group.add_argument(
        ...
        action="store",
        default="",
        help="Append to the default {nmap_extra} variable in scans.",
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Disable sanity checks that would otherwise prevent the scans from running. Default: false",
    )
    ...
    if args.concurrent_targets <= 0:
        error(
            "Argument -ch/--concurrent-targets: must be at least 1."
        )
        errors = True
    ...
    if concurrent_scans <= 0:
        error(
            "Argument -ct/--concurrent-scans: must be at least 1."
        )
        errors = True
    ...
    for profile in port_scan_profiles_config:
        if profile == port_scan_profile:
            ...
            for scan in port_scan_profiles_config[profile]:
                if (
                    "service-detection"
                    not in port_scan_profiles_config[
                        profile
                    ][scan]
                ):
                    error(
                        "The {profile}.{scan} scan does not have a defined service-detection section. Every scan must at least have a service-detection section defined with a command and a corresponding pattern that extracts the protocol (TCP/UDP), port, and service from the result."
                    )
                    errors = True
                else:
                    if (
                        ...
                        error(
                            "The {profile}.{scan}.service-detection section does not have a command defined. Every service-detection section must have a command and a corresponding pattern that extracts the protocol (TCP/UDP), port, and service from the results."
                        )
                        errors = True
                    else:
                        if (
                            "{ports}"
                            in port_scan_profiles_config[
                                profile
                            ][scan]["service-detection"][
                                "command"
                            ]
                    ...
                    if (
                        ...
                        error(
                            "The {profile}.{scan}.service-detection section does not have a pattern defined. Every service-detection section must have a command and a corresponding pattern that extracts the protocol (TCP/UDP), port, and service from the results."
                        )
                        errors = True
                    else:
                        if not all(
    ...
    if not found_scan_profile:
        error(
            "Argument --profile: must reference a port scan profile defined in {port_scan_profiles_config_file}. No such profile found: {port_scan_profile}"
        )
        errors = True
    ...
    for target in args.targets:
        try:
            ...
            try:
                target_range = ipaddress.ip_network(
                    target, strict=False
                )
                if (
                    not args.disable_sanity_checks
                    and target_range.num_addresses > 256
                ):
                    error(
                        target
                        + " contains "
                ...
                try:
                    ...
                    error(
                        target
                        + " does not appear to be a valid IP address, IP range, or resolvable hostname."
                    )
                    errors = True
    ...
    with ProcessPoolExecutor(
        max_workers=args.concurrent_targets
    ) as executor:
        start_time = time.time()
        futures = []
        ...
        for address in targets:
            ...
            futures.append(
                executor.submit(
                    scan_host, target, concurrent_scans
                )
            )
        ...
        elapsed_time = calculate_elapsed_time(start_time)
        info(
            "{bgreen}Finished scanning all targets in {elapsed_time}!{rst}"
        )
